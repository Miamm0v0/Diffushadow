...
python plot_zz_vs_inverse_qubit.py `
  ...
  --distances 1 2 3 `
  --qubit_groups "Diffushadow1:8,10,12" `
...
def load_d_values(data, row_count):
    ...
    if values.shape[0] < row_count:
        raise ValueError(f"d_values has {values.shape[0]} entries, but ZZ data has {row_count} rows.")
...
def load_eval_result(file_path, args, explicit_n=None):
    ...
    d_values = load_d_values(data, zz_mean.shape[0])
...
def select_distances(results, requested_distances):
    ...
    if requested_distances is None:
        if not available:
            raise ValueError("No common saved ZZ distances across files.")
        return available
...
def collect_series(results, g_value, distance, tol, include_std):
    ...
    for result in results:
        ...
        ys.append(result.zz_mean[distance_index, index])
        if include_std and result.zz_std is not None:
            yerrs.append(result.zz_std[distance_index, index])
        else:
            yerrs.append(np.nan)
...
def save_separate_plot(results, g_value, distance, args, group_by_n, group_colors):
...
def save_by_distance_plot(results, g_values, distance, args, group_by_n, group_colors):
    fig, ax = plt.subplots(figsize=(args.fig_width, args.fig_height))
    fit_records = []
