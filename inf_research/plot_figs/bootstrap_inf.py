...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional d/component values for [B,D,H] observables. Files that do "
            "not contain every requested distance are skipped for that observable."
        ),
    )
...
def select_results_for_components(
    results: Sequence[BootstrapResult],
    key: str,
    requested: Optional[Sequence[int]],
) -> tuple[List[BootstrapResult], np.ndarray]:
    """Select component labels and the N files that contain them.

    Snapshot bootstrap files may contain different d_values for different
    system sizes. An explicitly requested distance therefore selects the
    subset of N files in which that distance was saved. Without an explicit
    request, only distances common to every input file are fitted.
    """
    if results[0].arrays[key].ndim == 2:
        return list(results), np.asarray([-1], dtype=np.int64)

    labels_by_result: List[np.ndarray] = []
    for result in results:
        values = result.arrays[key]
        if values.ndim != 3:
            raise ValueError(
                "{}:{} dimensionality differs across N files.".format(result.path, key)
            )
        count = values.shape[1]
        labels = (
            result.d_values
        ...
        labels_by_result.append(labels)

    if requested is None:
        common = set(int(value) for value in labels_by_result[0])
        for labels in labels_by_result[1:]:
            common.intersection_update(int(value) for value in labels)
        selected_labels = np.asarray(
            [int(value) for value in labels_by_result[0] if int(value) in common],
            dtype=np.int64,
        )
        if selected_labels.size == 0:
            raise ValueError(
                "No distance/component for '{}' is common to every input file; "
                "select one with --distance d.".format(key)
            )
        return list(results), selected_labels

    selected_labels = np.asarray(
        list(dict.fromkeys(int(value) for value in requested)), dtype=np.int64
    )
    eligible: List[BootstrapResult] = []
    for result, labels in zip(results, labels_by_result):
        available = set(int(value) for value in labels)
        if all(int(value) in available for value in selected_labels):
            eligible.append(result)
    if not eligible:
        available_by_n = {
            result.num_qubits: [int(value) for value in labels]
            for result, labels in zip(results, labels_by_result)
        }
        raise ValueError(
            "No input file contains requested distance/component values {} for '{}'. "
            "Available by N: {}.".format(selected_labels.tolist(), key, available_by_n)
        )
    return eligible, selected_labels
...
def align_finite_bootstrap(
    ...
    selected_component_labels: np.ndarray,
    bootstrap_count: int,
    ...
    for result in results:
        ...
        if values.ndim == 2:
            ...
            selected_components, labels = component_indices(
                result, key, selected_component_labels.tolist()
            )
            if not np.array_equal(labels, selected_component_labels):
                raise ValueError(
                    "{}:{} selected component labels changed unexpectedly.".format(
                        result.path, key
                    )
                )
...
def get_label(key: str, component: int) -> str:
    """根据观测键和组件返回自定义标签"""
    label_map = {
        "bootstrap_zz": r"$\langle Z_i Z_{i+r} \rangle$",
        "bootstrap_xx": r"$\langle X_i X_{i+1} \rangle$",
        "bootstrap_yy": r"$\langle Y_i Y_{i+1} \rangle$",
        "bootstrap_energy": r"$E_0/N$",
        "bootstrap_entanglement": r"$S_{\mathrm{ent}}$",
        "bootstrap_corr_spin_dot": r"$\langle \mathbf{S}_i \cdot \mathbf{S}_{i+1} \rangle$",
    }
    base = label_map.get(key, key)
    if component < 0:
        return base
    return r"{} (r={})".format(base, component)
...
def save_plot(
    ...
    for component_index, component in enumerate(component_labels):
        # label = key if component < 0 else "{} (r={})".format(key, int(component))
        label = get_label(key, component)
    ...
    axis.set_ylabel(r"$\langle Z_iZ_{i+r}\rangle_{N\rightarrow\infty}$")
...
def main() -> None:
    ...
    for key in observable_keys:
        key_results, component_labels = select_results_for_components(
            results, key, args.components
        )
        if len(key_results) < args.fit_degree + 1:
            raise ValueError(
                "Degree-{} finite-size fit of '{}' at distances/components {} needs "
                "at least {} usable N values; got {}: {}.".format(
                    args.fit_degree,
                    key,
                    component_labels.tolist(),
                    args.fit_degree + 1,
                    len(key_results),
                    [item.num_qubits for item in key_results],
                )
            )
        key_sizes = np.asarray(
            [item.num_qubits for item in key_results], dtype=np.int64
        )
        key_inverse_sizes = 1.0 / key_sizes.astype(np.float64)
        key_size_set = set(key_sizes.tolist())
        skipped_sizes = [
            item.num_qubits for item in results
            if item.num_qubits not in key_size_set
        ]
        bootstrap_count = select_bootstrap_count(
            key_results, key, args.bootstrap_repetitions
        )
        ...
        intercepts, fit_of_mean, slopes, central_r2 = fit_bootstrap_intercepts(
            key_inverse_sizes, finite, args.fit_degree
        )
        ...
        npz_output["{}_N_values".format(suffix)] = key_sizes
        npz_output["{}_inverse_N_values".format(suffix)] = key_inverse_sizes
        npz_output["{}_source_files".format(suffix)] = np.asarray(
            [os.path.abspath(item.path) for item in key_results]
        )
        ...
        for component_index, component in enumerate(component_labels):
            for param_index, param in enumerate(target_params):
                rows.append({
                    ...
                    "qubits": format_values(key_sizes),
                    "inverse_qubits": format_values(key_inverse_sizes),
        ...
        print(
            "Fitted {}: B={}, distances/components={}, N={}, parameters={}".format(
                key,
                bootstrap_count,
                component_labels.tolist(),
                key_sizes.tolist(),
        ...
        if skipped_sizes:
            print(
                "Skipped N without all requested distances/components for {}: {}".format(
                    key, skipped_sizes
                )
            )
